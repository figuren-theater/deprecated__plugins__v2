...
export default function edit( {
	 ...
	 *
	 * @package theatrebase_production_blocks
	 * @version 2022.05.22
	 * @author  Carsten Bach
	 *
	...
	const DurationServerSideRender = () => (
	    <ServerSideRender
	        block="theatrebase/theatrebase-production-duration"
	        attributes={ {
	        	textAlign, 
	        	prefix, 
	        	suffix, 
		...
		<>
			<InspectorControls>
				<div className="block-editor-block-card" >
					<ToggleControl
						label={ __( 'Show human readable duration.', 'theatrebase-production-blocks' ) }
						onChange={ ( newHumanReadable ) => setAttributes( { humanReadable: newHumanReadable } ) }
						checked={ humanReadable }
			            help={
			                humanReadable
			                    ? __('Shows e.g. "2 hours 10 minutes".','theatrebase-production-blocks')
			                    : __('Shows pure "130" minutes only.','theatrebase-production-blocks')
			            }
					/>
				</div>
			</InspectorControls>
